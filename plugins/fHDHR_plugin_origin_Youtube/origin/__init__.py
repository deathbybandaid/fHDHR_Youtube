...
class Plugin_OBJ():
    ...
    def get_channels(self):
        ...
        conf_channel_list = self.plugin_utils.config.dict['youtube']["streams"]
        ...
        for station in conf_channel_list:
            ...
            station_item = {}
            if station in list(self.plugin_utils.config.dict.keys()):
                for channel_key in ["number", "name", "videoid"]:
                    if channel_key in list(self.plugin_utils.config.dict[station]):
                        station_item[channel_key] = str(self.plugin_utils.config.dict[station][channel_key])
